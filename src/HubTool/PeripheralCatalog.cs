...
public static class PeripheralCatalog
{
    ...
    public static bool IsPeripheral(Device device) => device.Id.StartsWith("audio:") || device.Id.StartsWith("monitor:") || device.Id.StartsWith("camera:")
        || device.Id.StartsWith("input:") || device.Id.StartsWith("windows:") || IsNativePeripheral(device.Kind, device.Id, device.Name);

    public static bool IsVisible(Device device) => IsPeripheral(device) && !device.Id.StartsWith("windows:");
